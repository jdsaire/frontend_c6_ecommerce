...
covers Activities 1 through 4, plus a storefront-bridge run between Activities 2
...
reason — one screen's story, not a separate numbered file. Activity 5 is still a
separate, later delivery against this same repository, and its walkthrough file
lands when that code does.
...
| [`04-Input-Validation-and-Authentication.md`](04-Input-Validation-and-Authentication.md) | Input validation and sanitization in plain terms; why "prevent SQL injection" and "prevent XSS" mean something different when there's no SQL and Blazor already encodes by default; the search box and login form built with `EditForm`/`DataAnnotations`; simulated authentication with Blazor's real `AuthenticationStateProvider`/`AuthorizeView`; gating the cart on sign-in from the calling layer; and the checkout screen and its gating. |
